...
#Creamos la ventana raíz de la aplicacion
root = Tk() 
...
#Creacion del marco principal con el estilo que creamos anteriormente
...
for i in range(5): #El bucle for itera sobre las primeras 5 columnas del marco principal (índices 0, 1, 2, 3, 4).
    mainframe.columnconfigure(i, weight=1) #i es el índice de la columna que se está configurando
for i in range(8):
    mainframe.rowconfigure(i, weight=1)

#Definimos y configuramos el estilo de los widgets
...
#Creamos y configuramos las entradas de la calculadora
...
#Configuramos el estilo para los botones númericos
...
#Creamos los botones
#Creamos un botón en el marco principal (mainframe) con el texto "0". Se aplica el estilo "Botones_numero.TButton" y se asocia la función IngresarValores("0") al evento de clic del botón
...
boton_raiz_cuadrada = ttk.Button(mainframe, text="√", style="Botones_funciones.TButton", command=lambda: raizCuadrada())
...
boton_porcentaje = ttk.Button(mainframe, text="%", style="Botones_funciones.TButton", command=lambda: porcentaje())

#Ubicacion de los botones en la pantalla
